...
LLMWHISPERER_API_KEY_3 = os.getenv("LLMWHISPERER_API_KEY_3")
...
PROGRAMAS_VERDES = {
    ...
    "derecho",
    "administraciòn financiera virtual",
...
ABREVIATURAS_PREDEFINIDAS = {
    ...
    "ESPECIALIZACIÓN EN DESAROLLO INTEGRAL DE LA INFANCIA Y LA ADOLESCENCIA - EDIIA": "ESP DESAR INTGRL INFA Y ADOLES",
...
ABREVIATURAS_NORMALIZADAS = {
    normalizar_texto_para_busqueda(k): v for k, v in ABREVIATURAS_PREDEFINIDAS.items()
}
...
def _intentar_extraccion_llmwhisperer(file_path, api_key, key_number):
    ...
    print(
        f"Inicializando cliente LLMWhisperer para PDF '{nombre_base}' con API Key #{key_number}..."
    )
    client = LLMWhispererClientV2(api_key=api_key)
    print(f"Enviando PDF '{file_path}' a LLMWhisperer (Key #{key_number})...")
    resultado = client.whisper(
        file_path=file_path, wait_for_completion=True, wait_timeout=360
    )
    print(
    ...
    if (
        ...
        and "extraction" in resultado
        and isinstance(resultado["extraction"], dict)
        and "result_text" in resultado["extraction"]
    ):
        return resultado["extraction"]["result_text"].replace(
            "<<<\x0c", "\n--- NUEVA PÁGINA --- \n"
        )
    else:
...
def extraer_texto_pdf_con_apis(file_path):
    ...
    if current_api_key_index == 1:
        ...
        print(f"INFO: Usando API Key de respaldo #{current_key_num}...")
    else:
        current_key, current_key_num = LLMWHISPERER_API_KEY_3, 3
        print(f"INFO: Usando API Key de respaldo #{current_key_num}...")

    try:
        return _intentar_extraccion_llmwhisperer(
            file_path, current_key, current_key_num
        )
    except LLMWhispererClientException as e:
        msg_error_api = str(e).lower()
        codigo_estado_api = e.status_code if hasattr(e, "status_code") else 0

        # Detectar error de límite alcanzado
        if (
            codigo_estado_api == 402
            or "breached your free processing limit" in msg_error_api
        ):
            # --- CASO 1: Falló la Key 1 -> Intentar Key 2 ---
            if current_api_key_index == 1:
                print(
                    f"⚠️ LÍMITE API #1. Cambiando a API #2 para '{os.path.basename(file_path)}'..."
                )
                with thread_lock:
                    mensajes_resumen_procesamiento.append(
                        f"⚠️ LÍMITE API Unstract #1. Cambiando a API #2."
                    )
                    globals()["active_api_key_index"] = 2

                try:
                    # Intento inmediato con Key 2
                    return _intentar_extraccion_llmwhisperer(
                        file_path, LLMWHISPERER_API_KEY_2, 2
                    )
                except LLMWhispererClientException as e2:
                    # Si la Key 2 TAMBIÉN falla por límite inmediatamente, pasar a la 3
                    code2 = e2.status_code if hasattr(e2, "status_code") else 0
                    if code2 == 402 or "breached" in str(e2).lower():
                        print(
                            f"⚠️ LÍMITE API #2 también alcanzado. Cambiando a API #3..."
                        )
                        with thread_lock:
                            mensajes_resumen_procesamiento.append(
                                f"⚠️ LÍMITE API Unstract #2. Cambiando a API #3."
                            )
                            globals()["active_api_key_index"] = 3
                        return _intentar_extraccion_llmwhisperer(
                            file_path, LLMWHISPERER_API_KEY_3, 3
                        )
                    else:
                        raise e2  # Fue otro tipo de error en la key 2

            # --- CASO 2: Falló la Key 2 -> Intentar Key 3 ---
            elif current_api_key_index == 2:
                print(
                    f"⚠️ LÍMITE API #2. Cambiando a API #3 para '{os.path.basename(file_path)}'..."
                )
                with thread_lock:
                    mensajes_resumen_procesamiento.append(
                        f"⚠️ LÍMITE API Unstract #2. Cambiando a API #3."
            ...
            # --- CASO 3: Falló la Key 3 -> Fin del camino ---
            else:  # current_api_key_index == 3
                msg = (
                    f"❌ LÍMITE ALCANZADO en API Key #3. No hay más claves disponibles."
                )
                print(msg)
                with thread_lock:
                    mensajes_resumen_procesamiento.append(msg)
...
def extraer_texto_excel_con_pandas(file_path):
    ...
    try:
        # MODIFICACIÓN: Usamos 'with' para asegurar que el archivo se cierre al terminar este bloque
        ...
        # Aquí el archivo ya está cerrado automáticamente gracias al 'with'
...
def analizar_con_gemini(texto_documento, pregunta_o_instruccion, nombre_archivo_base):
    ...
    try:
        ...
        print(
            f"✅ Respuesta recibida de Google Gemini API para '{nombre_archivo_base}'."
        )
...
def extraer_datos_completos(respuesta_llm):
    ...
    if match_nombre:
        datos["Nombre"] = limpiar_valor(match_nombre.group(1).splitlines()[0])

    match_programa = re.search(r"PROGRAMA_ASPIRA:\s*(.*)", respuesta_llm)
    if match_programa:
        datos["Programa"] = limpiar_valor(match_programa.group(1).splitlines()[0])

    match_plan = re.search(r"PLAN_ESTUDIO:\s*(.*)", respuesta_llm)
    if match_plan:
        datos["Plan"] = (
            limpiar_valor(match_plan.group(1).strip().splitlines()[0]) or "N/A"
        )

    match_origen = re.search(r"NOMBRE_PROGRAMA_ORIGEN:\s*(.*)", respuesta_llm)
    if match_origen:
        datos["Programa Origen"] = limpiar_valor(
            match_origen.group(1).strip().splitlines()[0]
        )

    match_abrev = re.search(r"ABREVIACION_SUGERIDA:\s*(.*)", respuesta_llm)
    if match_abrev:
        datos["Abreviacion Sugerida"] = limpiar_valor(
            match_abrev.group(1).strip().splitlines()[0]
        )
    ...
    match_script = re.search(
        r"--- INICIO SCRIPT AUTOHOTKEY ---(.*?)--- FIN SCRIPT AUTOHOTKEY ---",
        respuesta_llm,
        re.DOTALL,
    )
    if match_script:
...
def limpiar_marcadores_codigo(texto):
    """Elimina los marcadores de código markdown (```) de la primera y última línea si existen."""
    if not texto:
        return texto
    lineas = texto.splitlines()

    # Eliminar primera línea si es ```
...
def save_unprocessed_files(files_to_save, parent_window):
    if not files_to_save:
        messagebox.showinfo(
            "Información",
            "No hay archivos no procesados para guardar.",
            parent=parent_window,
        )
        return

    dest_folder = filedialog.askdirectory(
        title="Seleccione una carpeta para guardar los archivos no procesados"
    )
    if not dest_folder:
        return
...
def guardar_resultados_csv(datos_a_guardar, directorio_salida, ventana_padre):
    ...
    try:
        ...
        if not os.path.exists(directorio_logs):
            print(f"La carpeta 'LOGS' no existe. Creándola en: {directorio_logs}")
...
def mostrar_resumen_log_gui(main_root_ref, lista_mensajes, unprocessed_list):
    ...
    close_button = ttk.Button(
        button_frame, text="Cerrar Log", command=resumen_ventana.destroy
    )
...
def actualizar_fila_en_tabla(ventana_principal, tabla_treeview, datos_fila):
    def actualizar():
        ...
        with thread_lock:
            ...
            processed_data_cache[:] = [
                d
                for d in processed_data_cache
                if d.get("Ruta Completa") != ruta_a_buscar
            ]
            processed_data_cache.append(datos_fila)

        if not tabla_treeview.exists(fila_id):
            print(
                f"ADVERTENCIA: Se intentó actualizar la fila para '{os.path.basename(fila_id)}' pero no se encontró."
            )
            return
...
def crear_tabla_resumen_en_vivo(main_root_ref, retry_callback):
    ...
    style.configure(
        "Custom.Treeview", rowheight=30, font=("Segoe UI", 9), fieldbackground="white"
    )
    ...
    for col in cols:
        tree.heading(
            col, text=col, command=lambda _col=col: sort_by_column(tree, _col, False)
        )
        if col == "Acciones":
        ...
        elif col == "Modelo Usado":
            tree.column(col, width=150, minwidth=120, anchor="w")
        elif col == "Ejecutar Script":
            tree.column(col, width=150, minwidth=130, anchor="center")
        elif col == "Marcar Realizada":
            tree.column(col, width=150, minwidth=130, anchor="center")
        elif col == "Marcar Pendiente":
            tree.column(col, width=150, minwidth=130, anchor="center")
        else:
    ...
    def on_click(event):
        ...
        if column_id_str == "#6":
            if os.path.exists(row_id):
                try:
                    if os.name == "nt":
                        ...
                        subprocess.run(["xdg-open", row_id], check=True)
                    else:
                        subprocess.run(["open", row_id], check=True)
                except Exception as e:
                    messagebox.showerror(
                        "Error",
                        f"No se pudo abrir el archivo original:\n{e}",
                        parent=tabla_ventana,
                    )
            else:
                messagebox.showwarning(
                    "Archivo no encontrado",
                    f"No se encontró el archivo original:\n{row_id}",
                    parent=tabla_ventana,
                )

        elif column_id_str == "#7":
            with thread_lock:
                datos_del_archivo = next(
                    (
                        item
                        for item in processed_data_cache
                        if item.get("Ruta Completa") == row_id
                    ),
                    None,
                )
                ...
                if ruta_ahk and os.path.exists(ruta_ahk):
                    try:
                        if os.name == "nt":
                            ...
                            messagebox.showinfo(
                                "Información",
                                "La ejecución de scripts AHK solo es compatible con Windows.",
                                parent=tabla_ventana,
                            )
                    except Exception as e:
                        messagebox.showerror(
                            "Error",
                            f"No se pudo ejecutar el script AHK:\n{e}",
                            parent=tabla_ventana,
                        )
                else:
                    messagebox.showwarning(
                        "Script no Encontrado",
                        "No se encontró un script AHK para este archivo.",
                        parent=tabla_ventana,
                    )
            else:
                messagebox.showwarning(
                    "Datos no encontrados",
                    "No se encontraron datos de procesamiento para este archivo.",
                    parent=tabla_ventana,
                )
            ...
            if not pdf_path or not os.path.exists(pdf_path):
                messagebox.showerror(
                    "Error",
                    "No se encontró la ruta del archivo PDF original.",
                    parent=tabla_ventana,
                )
                return
            ...
            try:
                ...
                messagebox.showinfo(
                    "Éxito",
                    f"Archivos movidos correctamente:\n\nPDF a: {realizadas_dir}\nScript AHK a: {scripts_dir}",
                    parent=tabla_ventana,
                )
                ...
                messagebox.showerror(
                    "Error al Mover",
                    f"Ocurrió un error al mover los archivos:\n{e}",
                    parent=tabla_ventana,
                )
            ...
            dialogo.geometry("650x300")
            dialogo.resizable(False, False)

            # Hacer la ventana modal (bloquea la de atrás hasta cerrar)
            ...
            ttk.Label(
                frame_diag,
                text="Agregue el detalle al final del nombre:",
                font=("Segoe UI", 10),
            ...
            # Parte 1: Nombre original (Estático)
            lbl_orig = ttk.Label(
                frame_nombre, text=nombre_sin_ext, font=("Segoe UI", 9, "bold")
            )
            lbl_orig.pack(side=tk.LEFT)

            # Parte 2: El guión separador (Estático)
            lbl_sep = ttk.Label(frame_nombre, text=" - ", font=("Segoe UI", 9, "bold"))
            lbl_sep.pack(side=tk.LEFT)

            # Parte 3: Combobox para seleccionar o escribir
            combo_sufijo = ttk.Combobox(frame_nombre, width=35)
            combo_sufijo.pack(side=tk.LEFT, fill=tk.X, expand=True)
            combo_sufijo.focus_set()

            # Cargar textos predeterminados
            predefined_texts = load_predefined_texts()
            combo_sufijo["values"] = predefined_texts

            ttk.Label(
                frame_diag, text=f"Extensión: {extension}", foreground="gray"
            ).pack(anchor="w", pady=(2, 10))

            # Frame para botones de gestión de textos predeterminados
            frame_textos = ttk.LabelFrame(
                frame_diag, text="Gestión de textos predeterminados", padding="5"
            )
            frame_textos.pack(fill=tk.X, pady=(5, 10))

            def guardar_texto_predeterminado():
                texto = combo_sufijo.get().strip()
                if not texto:
                    messagebox.showwarning(
                        "Sin texto",
                        "Escriba un texto primero para guardarlo.",
                        parent=dialogo,
                    )
                    return

                predefined_texts = load_predefined_texts()
                if texto not in predefined_texts:
                    predefined_texts.append(texto)
                    save_predefined_texts(predefined_texts)
                    combo_sufijo["values"] = predefined_texts
                    messagebox.showinfo(
                        "Éxito", "Texto guardado correctamente.", parent=dialogo
                    )
                else:
                    messagebox.showinfo(
                        "Información",
                        "Este texto ya existe en la lista.",
                        parent=dialogo,
                    )

            def eliminar_texto_predeterminado():
                texto = combo_sufijo.get().strip()
                if not texto:
                    messagebox.showwarning(
                        "Sin selección",
                        "Seleccione un texto de la lista para eliminarlo.",
                        parent=dialogo,
                    )
                    return

                predefined_texts = load_predefined_texts()
                if texto in predefined_texts:
                    if messagebox.askyesno(
                        "Confirmar",
                        f"¿Eliminar '{texto}' de los textos predeterminados?",
                        parent=dialogo,
                    ):
                        predefined_texts.remove(texto)
                        save_predefined_texts(predefined_texts)
                        combo_sufijo["values"] = predefined_texts
                        combo_sufijo.set("")
                        messagebox.showinfo(
                            "Éxito", "Texto eliminado correctamente.", parent=dialogo
                        )
                else:
                    messagebox.showwarning(
                        "No encontrado",
                        "Este texto no está en la lista de predeterminados.",
                        parent=dialogo,
                    )

            btn_guardar_texto = ttk.Button(
                frame_textos,
                text="💾 Guardar texto",
                command=guardar_texto_predeterminado,
            )
            btn_guardar_texto.pack(side=tk.LEFT, padx=5)

            btn_eliminar_texto = ttk.Button(
                frame_textos,
                text="🗑️ Eliminar texto",
                command=eliminar_texto_predeterminado,
            )
            btn_eliminar_texto.pack(side=tk.LEFT, padx=5)

            # Función interna para ejecutar el movimiento
            def confirmar_accion(event=None):
                texto_agregado = combo_sufijo.get().strip()

                # Validar caracteres inválidos en nombres de archivo
                caracteres_invalidos = '<>:"/\\|?*'
                if any(c in caracteres_invalidos for c in texto_agregado):
                    messagebox.showwarning(
                        "Nombre inválido",
                        f"El texto no puede contener: {caracteres_invalidos}",
                        parent=dialogo,
                    )
                    return
                ...
                if texto_agregado:
                    nuevo_nombre_archivo = (
                        f"{nombre_sin_ext} - {texto_agregado}{extension}"
                    )
                else:
                    # Si el usuario no escribe nada, confirmamos si quiere moverlo con el nombre original
                    if not messagebox.askyesno(
                        "Sin texto",
                        "¿Desea mover el archivo sin agregar ningún texto adicional?",
                        parent=dialogo,
                    ):
                        return
                ...
                try:
                    ...
                    nueva_ruta_completa = os.path.join(
                        pendientes_dir, nuevo_nombre_archivo
                    )
                    ...
                    if os.path.exists(nueva_ruta_completa):
                        messagebox.showerror(
                            "Error",
                            f"Ya existe un archivo con este nombre en la carpeta PENDIENTES:\n{nuevo_nombre_archivo}",
                            parent=dialogo,
                        )
                        return
                    ...
                    # Éxito: Eliminar de la tabla y cerrar diálogo
                    tree.delete(row_id)
                    dialogo.destroy()
                    messagebox.showinfo(
                        "Éxito",
                        f"Archivo renombrado y movido a:\n{nueva_ruta_completa}",
                        parent=tabla_ventana,
                    )
                    ...
                    messagebox.showerror(
                        "Error al Mover",
                        f"Ocurrió un error al mover el archivo:\n{e}",
                        parent=dialogo,
                    )
            ...
            btn_ok = ttk.Button(
                frame_btn, text="Aceptar y Mover", command=confirmar_accion
            )
    ...
    def editar_con_bloc_notas(ruta_ahk):
        ...
        if not ruta_ahk or not os.path.exists(ruta_ahk):
            messagebox.showerror(
                "Error",
                "El archivo de script no existe o no se encuentra.",
                parent=tabla_ventana,
            )
            return
        try:
            # Usamos Popen para que no congele la interfaz mientras el bloc de notas está abierto
            subprocess.Popen(["notepad.exe", ruta_ahk])
        except Exception as e:
            messagebox.showerror(
                "Error",
                f"No se pudo iniciar el Bloc de notas:\n{e}",
                parent=tabla_ventana,
            )

    def on_right_click(event):
        """Maneja el clic derecho para mostrar el menú de edición"""
        ...
        if col_id == "#7":
            ...
            # Buscar la ruta del AHK en la caché
            with thread_lock:
                datos_archivo = next(
                    (
                        item
            ...
            if datos_archivo and datos_archivo.get("Ruta AHK"):
                ...
                menu_contextual.add_command(
                    label="✏️ Editar con Bloc de notas",
                ...
                # Opcional: Mostrar mensaje si no hay script asociado aún
                pass
...
def worker_process_file(
    ...
    while not archivo_queue.empty():
        ...
        ventana_principal.after(
            0,
            lambda n=nombre_base_archivo,
            m=model_a_usar.split("/")[0]: etiqueta_estado.config(
                text=f"Procesando: {n} (con {m})"
            ),
        )
        mensaje_log_actual = [
        ...
        if texto_extraido:
            ...
            if texto_limpio_para_llm:
                ...
                mensaje_log_actual.append(
                    f"INFO: Nivel académico clasificado como: {nivel_academico}."
                )
                prompt_a_usar = (
                    PROMPT_ANALISIS_POSTGRADO
                    if nivel_academico == "Posgrado"
                    else PROMPT_ANALISIS_DOCUMENTO
                )
                if nivel_academico == "Posgrado":
                    mensaje_log_actual.append(
                        "INFO: Usando prompt especializado para Postgrado."
                    )
                ...
                try:
                    ...
                    mensaje_log_actual.append(
                        f"❌ ERROR INESPERADO con el modelo {model_a_usar}: {e}"
                    )
                    respuesta_llm = None
                ...
                if respuesta_llm:
                    ...
                    if "DETENER_PROCESO_CONDICION_" in respuesta_llm:
                        for line in respuesta_llm.splitlines():
                            if "DETENER_PROCESO_CONDICION_" in line:
                                ...
                                if "PERIODO" in detalle:
                                    datos_para_fila_actual["Plan"] = "DETENIDO: PERIODO"
                                elif "PROGRAMA" in detalle:
                                    datos_para_fila_actual["Plan"] = (
                                        "DETENIDO: PROGRAMA"
                                    )
                                elif "CALIFICACION" in detalle:
                                    datos_para_fila_actual["Plan"] = (
                                        "DETENIDO: CALIFICACION"
                                    )
                                else:
                                    datos_para_fila_actual["Plan"] = "DETENIDO: OTRO"
                                break
                    else:
                        if (
                            datos_completos["Script Template"]
                            and datos_completos.get("Nombre") != "No extraído"
                        ):
                            ...
                            if len(programa_origen) <= 30:
                                ...
                                mensaje_log_actual.append(
                                    f"INFO: Abreviatura: Usando nombre de programa de origen original (<=30 caracteres)."
                                )
                            else:
                                nombre_origen_norm = normalizar_texto_para_busqueda(
                                    programa_origen
                                )
                                if nombre_origen_norm in ABREVIATURAS_NORMALIZADAS:
                                    abreviatura = ABREVIATURAS_NORMALIZADAS[
                                        nombre_origen_norm
                                    ]
                                    mensaje_log_actual.append(
                                        f"INFO: Abreviatura: Encontrada en la lista predefinida."
                                    )
                                else:
                                    abreviatura = datos_completos[
                                        "Abreviacion Sugerida"
                                    ]
                                    mensaje_log_actual.append(
                                        f"INFO: Abreviatura: Usando sugerencia del LLM (no encontrada en predefinidas)."
                                    )
                            ...
                            script_final = datos_completos["Script Template"].replace(
                                "(LINEA_ENCABEZADO_AQUI)", encabezado
                            )

                            nombre_sanitizado = sanitizar_nombre_archivo(
                                datos_completos["Nombre"]
                            )
                            ruta_ahk = os.path.join(
                            ...
                            try:
                                ...
                                mensaje_log_actual.append(
                                    f"✅ Script AHK guardado en: {ruta_ahk}"
                                )
                    ...
                    mensaje_log_actual.append(
                        f"❌ Falló el API del modelo {model_a_usar}. No se pudo procesar '{nombre_base_archivo}'."
                    )
                    datos_para_fila_actual.update(
                        {
                            "Nombre": f"Error en API ({model_a_usar.split('/')[0]})",
                            "Programa": "N/A",
                            "Plan": "N/A",
                        }
                    )
                ...
                datos_para_fila_actual.update(
                    {"Nombre": "Texto vacío", "Programa": "N/A", "Plan": "N/A"}
                )
        else:
            datos_para_fila_actual.update(
                {"Nombre": "Error Extracción", "Programa": "N/A", "Plan": "N/A"}
            )
        ...
        if not is_processed_successfully:
            with thread_lock:
                failure_info = {
                    "path": archivo_actual,
                    "failed_model": "EXTRACTION_FAILURE"
                    if not texto_extraido
                    else model_a_usar,
                }
                if not any(
                    d["path"] == failure_info["path"] for d in unprocessed_files_paths
                ):
                    unprocessed_files_paths.append(failure_info)

        actualizar_fila_en_tabla(
            ventana_principal, tabla_treeview, datos_para_fila_actual
        )
...
def procesar_archivos_seleccionados(
    lista_archivos,
    boton_seleccionar,
    boton_ver_tabla,
    etiqueta_estado,
    ...
    if etiqueta_etr:
        etiqueta_etr.config(
            text=f"Procesando con {num_modelos_seleccionados} workers seleccionados..."
        )
...
def iniciar_procesamiento_en_hilo(
    boton_seleccionar,
    boton_ver_tabla,
    etiqueta_estado,
    barra_progreso,
    ...
    if not selected_models:
        messagebox.showerror(
            "Sin Modelos",
            "Debe seleccionar al menos un modelo para procesar.",
            parent=ventana_principal,
        )
        return
    ...
    def iniciar_reintento():
        ...
        selected_models_for_retry = [
            model_name for var, model_name in model_vars if var.get()
        ]
        if not selected_models_for_retry:
            messagebox.showerror(
                "Sin Modelos",
                "Debe seleccionar al menos un modelo para el reintento.",
                parent=ventana_principal,
            )
            return
        ...
        if not files_to_retry_info:
            messagebox.showinfo(
                "Información",
                "No hay archivos fallidos para reintentar.",
                parent=ventana_principal,
            )
            return
        ...
        if retry_button_ref:
            retry_button_ref.config(state=tk.DISABLED)
    ...
    if boton_seleccionar:
        boton_seleccionar.config(state=tk.DISABLED)
    if etiqueta_estado:
        etiqueta_estado.config(text="Seleccionando archivos...")
    ...
    if not file_paths:
        if etiqueta_estado:
            etiqueta_estado.config(text="Ningún archivo seleccionado.")
        if boton_seleccionar:
            boton_seleccionar.config(state=tk.NORMAL)
        return
    ...
    try:
        lista_ordenada = sorted(
            list(file_paths), key=lambda r: os.path.getmtime(r), reverse=False
        )
        ...
        for ruta_archivo in lista_ordenada:
            ...
            tabla_treeview.insert(
                "",
                "end",
                values=valores_iniciales,
                tags=tags_iniciales,
                iid=ruta_archivo,
            )
        ...
        if etiqueta_estado:
            etiqueta_estado.config(text="Error al procesar.")
        if boton_seleccionar:
            boton_seleccionar.config(state=tk.NORMAL)
...
def show_last_results_table(main_root_ref):
    if not processed_data_cache:
        messagebox.showinfo(
            "Sin Resultados",
            "Aún no se ha procesado ningún archivo en esta sesión.",
            parent=main_root_ref,
        )
        return
    ...
    for datos_fila in processed_data_cache:
        ...
        tabla_recreada.insert(
            "",
            "end",
            values=valores,
            tags=tags_finales,
            iid=datos_fila.get("Ruta Completa"),
        )
...
2.  **TABLA DE DATOS (Materias):**
    ...
    - Si la calificación tiene dos decimales y el segundo decimal no es un cero, entonces se debe aproximar y dejar siempre con un cero como segundo decimal (por ejemplo, 4.45 se convierte en 4.50) 
...
def save_notes(text_widget, parent_window):
    ...
    try:
        ...
        messagebox.showinfo(
            "Notas Guardadas",
            f"Sus notas se han guardado correctamente en:\n{NOTES_FILE}",
            parent=parent_window,
        )
    except Exception as e:
        messagebox.showerror(
...
# --- CONFIGURACIÓN DE PERSISTENCIA DE MODELOS ---
...
def load_model_config():
    ...
    try:
        ...
        print(
            f"Advertencia: No se pudo cargar la configuración de modelos. Usando valores por defecto. Error: {e}"
        )
    return {}
...
PREDEFINED_TEXTS_FILE = os.path.join(
    os.path.expanduser("~"), "ahk_predefined_texts.json"
)


def load_predefined_texts():
    """Carga los textos predeterminados para pendientes desde un archivo JSON."""
    try:
        if os.path.exists(PREDEFINED_TEXTS_FILE):
            with open(PREDEFINED_TEXTS_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception as e:
        print(
            f"Advertencia: No se pudieron cargar los textos predeterminados. Error: {e}"
        )
    return []


def save_predefined_texts(texts_list):
    """Guarda los textos predeterminados en un archivo JSON."""
    try:
        with open(PREDEFINED_TEXTS_FILE, "w", encoding="utf-8") as f:
            json.dump(texts_list, f, indent=4, ensure_ascii=False)
        print(f"Textos predeterminados guardados en: {PREDEFINED_TEXTS_FILE}")
    except Exception as e:
        print(f"Error al guardar los textos predeterminados: {e}")


if __name__ == "__main__":
    ventana_principal_app = tk.Tk()
    ventana_principal_app.title("Procesador de Documentos AHK")
    # Geometría ajustada para incluir la sección de notas
    ventana_principal_app.geometry("950x600")

    # --- MODIFICADO: Contenedor principal para diseño de dos columnas ---
    ...
    # Frame derecho para la nueva sección de notas
    ...
    etiqueta_estado_gui = ttk.Label(
        frame_izquierdo, text="Listo para iniciar.", font=("Segoe UI", 10)
    )
    ...
    deselect_all_button = ttk.Button(
        model_button_frame, text="Deseleccionar Todos", command=deselect_all_models
    )
    deselect_all_button.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))

    barra_progreso_gui = ttk.Progressbar(
        frame_izquierdo, orient="horizontal", length=300, mode="determinate"
    )
    barra_progreso_gui.pack(pady=10)
    etiqueta_etr_gui = ttk.Label(
        frame_izquierdo, text="Tiempo restante: --:--", font=("Segoe UI", 9)
    )
    ...
    notes_text_area = scrolledtext.ScrolledText(
        notes_frame, wrap=tk.WORD, width=40, height=20, font=("Segoe UI", 9)
    )
    ...
    # --- Verificación de claves API (sin cambios) ---
    keys_faltantes = []
    if not LLMWHISPERER_API_KEY_1:
        keys_faltantes.append("LLMWHISPERER_API_KEY_1")
    if not LLMWHISPERER_API_KEY_3:
        keys_faltantes.append("LLMWHISPERER_API_KEY_3")
    if not OPENROUTER_API_KEY:
        keys_faltantes.append("OPENROUTER_API_KEY")
    if not GEMINI_API_KEY:
        keys_faltantes.append("GEMINI_API_KEY")
    ...
    if keys_faltantes:
        ...
        messagebox.showerror(
            "Error de Configuración", mensaje_error, parent=ventana_principal_app
        )
    else:
        etiqueta_estado_gui.config(
            text=f"Claves OK. {len(WORKER_MODELS)} modelos activos detectados. Seleccione y procese."
        )
