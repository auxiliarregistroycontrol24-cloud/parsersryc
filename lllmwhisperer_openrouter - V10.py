...
# --- NUEVO: Definición del archivo de caché para persistencia de sesión ---
CACHE_FILE = os.path.join(os.path.expanduser('~'), "ahk_processor_session_cache.json")


# --- CONFIGURACIÓN DE WORKERS Y MODELOS (BASADO EN V9) ---
WORKER_MODELS = [
...
YOUR_SITE_NAME = "Procesador AHK Unificado"
...
ABREVIATURAS_PREDEFINIDAS = {
    ...
    "TECNOLOGIA EN GESTION DE SISTEMAS DE TELECOMUNICAÇÕES": "TG GSTON SISTEM DE TELECOMU",
    ...
    "TÉCNICO LABORAL POR COMPETENCIAS EN INSTALADOR DE REDES DE TELECOMUNICAÇÕES": "TC LAB COMPT INSTALD RED COMU",
...
# --- MÉTODO DE EXTRACCIÓN 1: UNSTRACT (DE V8) ---
...
# --- MÉTODO DE EXTRACCIÓN 2: PYMUPDF (DE V9) ---
def extraer_texto_pdf_con_pymupdf(file_path):
    global mensajes_resumen_procesamiento
    nombre_base = os.path.basename(file_path)
    print(f"Procesando archivo PDF '{nombre_base}' con PyMuPDF...")
    try:
        doc = fitz.open(file_path)
        md_output = to_markdown(doc)

        if isinstance(md_output, dict) and "text" in md_output:
            texto_markdown = md_output["text"]
        elif isinstance(md_output, str):
            texto_markdown = md_output
        else:
            raise TypeError(f"El resultado de to_markdown no es ni un diccionario con la clave 'text' ni un string. Tipo obtenido: {type(md_output)}")

        print(f"✅ Texto extraído y convertido a Markdown para el PDF '{nombre_base}'.")
        return texto_markdown
    except Exception as e:
        msg = f"❌ Error al procesar PDF '{nombre_base}' con PyMuPDF: {e}"
        print(msg)
        traceback.print_exc()
        with thread_lock:
            mensajes_resumen_procesamiento.append(msg + f"\n{traceback.format_exc()}")
...
# --- NUEVO: FUNCIÓN DESPACHADORA DE EXTRACCIÓN DE PDF ---
def extraer_texto_pdf(file_path, metodo):
    if metodo == "Unstract":
        return extraer_texto_pdf_con_apis(file_path)
    elif metodo == "PyMuPDF":
        return extraer_texto_pdf_con_pymupdf(file_path)
    else:
        msg = f"❌ Método de extracción desconocido: {metodo}"
        print(msg)
        with thread_lock:
            mensajes_resumen_procesamiento.append(msg)
        return None
...
def actualizar_fila_en_tabla(ventana_principal, tabla_treeview, datos_fila):
    def actualizar():
        ...
        if not tabla_treeview.exists(fila_id):
            print(f"ADVERTENCIA: Se intentó actualizar la fila para '{os.path.basename(fila_id)}' pero no se encontró.")
            return

        programa_original = datos_fila.get("Programa", "N/A")
        programa_normalizado = programa_original.lower().strip()
        
        # --- MODIFICACIÓN: Lógica de color con prioridad para psicología ---
        if 'psicologia' in programa_normalizado:
            tag_color = 'psicologia'
        elif programa_normalizado in PROGRAMAS_VERDES:
            tag_color = 'verde'
        else:
            tag_color = 'naranja'
...
def crear_tabla_resumen_en_vivo(main_root_ref, retry_callback):
    ...
    # --- MODIFICACIÓN: Añadido el nuevo tag de color para psicología ---
    tree.tag_configure('psicologia', background='#c6b1c9')
    ...
    def on_click(event):
        ...
        if column_id_str == '#6': 
            if os.path.exists(row_id):
                ...
                messagebox.showwarning("Archivo no encontrado", f"No se encontró el archivo original:\n{row_id}", parent=tabla_ventana)
                ...
                if ruta_ahk and os.path.exists(ruta_ahk):
                    try:
                        if os.name == 'nt':
                            ...
                            messagebox.showinfo("Información", "La ejecución de scripts AHK solo es compatible con Windows.", parent=tabla_ventana)
                    except Exception as e:
                        messagebox.showerror("Error", f"No se pudo ejecutar el script AHK:\n{e}", parent=tabla_ventana)
                else:
                    messagebox.showwarning("Script no Encontrado", "No se encontró un script AHK para este archivo.", parent=tabla_ventana)
            else:
            ...
            if not pdf_path or not os.path.exists(pdf_path):
                messagebox.showerror("Error", "No se encontró la ruta del archivo PDF original.", parent=tabla_ventana)
                return
            ...
            try:
                ...
                messagebox.showinfo("Éxito", f"Archivos movidos correctamente:\n\nPDF a: {realizadas_dir}\nScript AHK a: {scripts_dir}", parent=tabla_ventana)
                
                tree.delete(row_id)
            except Exception as e:
                messagebox.showerror("Error al Mover", f"Ocurrió un error al mover los archivos:\n{e}", parent=tabla_ventana)
            ...
            try:
                ...
                shutil.move(pdf_path, pendientes_dir)
                messagebox.showinfo("Éxito", f"Archivo movido a:\n{pendientes_dir}", parent=tabla_ventana)
                tree.delete(row_id)
            except Exception as e:
                messagebox.showerror("Error al Mover", f"Ocurrió un error al mover el archivo a 'PENDIENTES':\n{e}", parent=tabla_ventana)
...
def worker_process_file(archivo_queue, ventana_principal, tabla_treeview, barra_progreso, etiqueta_estado, num_archivos, model_a_usar, worker_id, metodo_extraccion_pdf, is_retry=False):
    ...
    while not archivo_queue.empty():
        ...
        if nombre_base_archivo.lower().endswith('.pdf'):
            # --- LÓGICA MODIFICADA: LLAMA A LA FUNCIÓN DESPACHADORA ---
            mensaje_log_actual.append(f"INFO: Usando el método de extracción de PDF: {metodo_extraccion_pdf}.")
            texto_extraido = extraer_texto_pdf(archivo_actual, metodo_extraccion_pdf)
...
def procesar_archivos_seleccionados(lista_archivos, boton_seleccionar, boton_ver_tabla, etiqueta_estado, barra_progreso, etiqueta_etr, ventana_principal, tabla_treeview, retry_button, selected_models, metodo_extraccion_pdf, is_retry=False):
    ...
    def procesar_en_paralelo():
        ...
        for i in range(num_workers_a_usar):
            ...
            thread = threading.Thread(
                ...
                # --- LÓGICA MODIFICADA: PASA EL MÉTODO DE EXTRACCIÓN AL WORKER ---
                args=(archivos_queue, ventana_principal, tabla_treeview, barra_progreso, etiqueta_estado, num_archivos, model_name, i + 1, metodo_extraccion_pdf, is_retry),
                daemon=True
            )
        ...
        def actualizar_gui_final():
            ...
            # --- NUEVO: Guardar resultados en el caché de sesión al finalizar ---
            save_results_to_cache(processed_data_cache)
...
def iniciar_procesamiento_en_hilo(boton_seleccionar, boton_ver_tabla, etiqueta_estado, barra_progreso, etiqueta_etr, ventana_principal, model_vars, extraction_method_var):
    
    selected_models = [model_name for var, model_name in model_vars if var.get()]
    metodo_seleccionado = extraction_method_var.get()

    if not selected_models:
        messagebox.showerror("Sin Modelos", "Debe seleccionar al menos un modelo para procesar.", parent=ventana_principal)
        return
        
    if metodo_seleccionado == "Unstract" and not LLMWHISPERER_API_KEY_1:
        messagebox.showerror("Clave API Faltante", "Ha seleccionado 'Unstract' pero la clave 'LLMWHISPERER_API_KEY_1' no está configurada en su archivo .env.", parent=ventana_principal)
        return
    ...
    def iniciar_reintento():
        ...
        procesar_archivos_seleccionados(
            ...
            selected_models_for_retry, metodo_seleccionado, is_retry=True
        )
    ...
    try:
        ...
        for ruta_archivo in lista_ordenada:
            ...
            valores_iniciales = (nombre_archivo, "Pendiente...", "", "", "", "📂 Ver Archivo", "", "✓ Realizada", "⏸ Pendiente")
        ...
        procesar_archivos_seleccionados(
            ...
            etiqueta_etr, ventana_principal, tabla_treeview, retry_button_ref, selected_models, metodo_seleccionado, is_retry=False
        )
...
def show_last_results_table(main_root_ref):
    if not processed_data_cache:
        messagebox.showinfo("Sin Resultados", "Aún no se ha procesado ningún archivo en esta sesión o no hay resultados guardados de sesiones anteriores.", parent=main_root_ref)
        return
...
Revise los siguientes datos del documento: el periodo académico y las calificaciones del "Programa Destino Ibero". Aplique las siguientes condiciones EN ORDEN. Si CUALQUIERA de estas condiciones se cumple, notifíquela claramente indicando la condición específica y la etiqueta de detención asociada, y NO continúe con los pasos de "PROCESAMIENTO DE DATOS" ni "FORMATO DE SALIDA".
...
# --- NUEVO: Funciones para guardar y cargar el caché de la sesión ---
def save_results_to_cache(data_cache):
    """Guarda la lista de resultados procesados en un archivo JSON."""
    try:
        with open(CACHE_FILE, 'w', encoding='utf-8') as f:
            json.dump(data_cache, f, indent=4)
        print(f"✅ Caché de la sesión guardado exitosamente en: {CACHE_FILE}")
    except Exception as e:
        print(f"❌ Error al guardar el caché de la sesión: {e}")

def load_results_from_cache():
    """Carga la lista de resultados procesados desde un archivo JSON si existe."""
    if not os.path.exists(CACHE_FILE):
        print("No se encontró un archivo de caché de sesión anterior.")
        return []
    try:
        with open(CACHE_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
            print(f"✅ Caché de la sesión anterior cargado desde: {CACHE_FILE}")
            return data
    except (json.JSONDecodeError, IOError) as e:
        print(f"❌ Error al cargar o decodificar el archivo de caché. Se iniciará una sesión nueva. Error: {e}")
        return []
...
if __name__ == "__main__":
    # --- MODIFICADO: Cargar resultados de la sesión anterior al iniciar ---
    processed_data_cache = load_results_from_cache()

    ventana_principal_app = tk.Tk()
    ventana_principal_app.title("Procesador Unificado de Documentos AHK v11.0")
    ...
    # --- NUEVO: SELECTOR DE MÉTODO DE EXTRACCIÓN ---
    extraction_frame = ttk.LabelFrame(frame_izquierdo, text="Método de Extracción de PDF", padding="10")
    extraction_frame.pack(pady=5, padx=10, fill=tk.X)
    
    extraction_method_var = tk.StringVar(value="PyMuPDF") # Valor por defecto
    
    rb_pymupdf = ttk.Radiobutton(extraction_frame, text="PyMuPDF (Local, Rápido)", variable=extraction_method_var, value="PyMuPDF")
    rb_pymupdf.pack(anchor='w', padx=5)

    rb_unstract = ttk.Radiobutton(extraction_frame, text="Unstract (API, Lento)", variable=extraction_method_var, value="Unstract")
    rb_unstract.pack(anchor='w', padx=5)


    model_frame = ttk.LabelFrame(frame_izquierdo, text="Selección de Modelos Activos", padding="10")
    ...
    # --- MODIFICADO: Habilitar el botón si se cargaron datos de la sesión anterior ---
    if processed_data_cache:
        boton_ver_tabla_gui.config(state=tk.NORMAL)
    else:
    ...
    boton_seleccionar_gui = ttk.Button(
        ...
        command=lambda: iniciar_procesamiento_en_hilo(
            ...
            etiqueta_etr_gui, ventana_principal_app, model_checkbox_vars, extraction_method_var
        )
    )
    ...
    keys_faltantes = []
    if not LLMWHISPERER_API_KEY_1: keys_faltantes.append("LLMWHISPERER_API_KEY_1 (Necesaria para método Unstract)")
    ...
    if keys_faltantes:
        ...
        # No se deshabilita el botón principal para permitir el uso del método local PyMuPDF
        if "OPENROUTER_API_KEY" in mensaje_error or "GEMINI_API_KEY" in mensaje_error:
