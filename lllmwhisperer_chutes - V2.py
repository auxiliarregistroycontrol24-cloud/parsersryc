...
def mostrar_tabla_resumen_gui(main_root_ref, datos_tabla): 
    ...
    window_width = 1050
    window_height = 450
    screen_width = tabla_ventana.winfo_screenwidth()
    screen_height = tabla_ventana.winfo_screenheight()
    ...
    for col in cols:
        ...
        if col == "Archivo Origen":
            tree.column(col, width=200, minwidth=150, anchor='w')
        elif col == "Nombre del estudiante":
            tree.column(col, width=200, minwidth=150, anchor='w')
    ...
    def on_click(event):
        ...
        if column == '#5':  # Columna de acciones
            ...
            if ruta_completa and os.path.exists(ruta_completa):
                ...
                messagebox.showwarning("Advertencia", "El archivo no existe o no está disponible.", parent=tabla_ventana)

    # Agregar estilo a los encabezados y filas
    style = ttk.Style()
    style.configure("Treeview.Heading", font=("Segoe UI", 10, "bold"))  # Fuente más clara en encabezados
    style.configure("Treeview", rowheight=25, font=("Segoe UI", 9))     # Ajuste de altura de filas

    # Aplicar color de fondo a la celda de "Acciones" para hacerla destacar como botón
    tree.tag_configure('accion', background="#E3F2FD")  # Color suave azul claro

    # Insertar filas con estilo
    for idx, item in enumerate(datos_tabla_ordenados):
        valores_fila = (
            item.get("Archivo Origen", "N/A"), 
            item.get("Nombre", "N/A"), 
        ...
        tree.insert("", "end", values=valores_fila, tags=('accion', item.get("Ruta Completa", "")))

    tree.bind("<Button-1>", on_click)

    btn_cerrar_tabla_final = ttk.Button(tabla_ventana, text="Cerrar Todo y Salir", command=main_root_ref.destroy) 
...
def procesar_archivos_seleccionados(lista_archivos, boton_seleccionar, etiqueta_estado, barra_progreso, etiqueta_etr, ventana_principal):
    ...
    def actualizar_estado(texto):
        ventana_principal.after(0, lambda: etiqueta_estado.config(text=texto))

    def actualizar_barra(valor):
        ventana_principal.after(0, lambda: barra_progreso.config(value=valor))

    def actualizar_etr(texto):
        ventana_principal.after(0, lambda: etiqueta_etr.config(text=texto))

    def desactivar_boton():
        ventana_principal.after(0, lambda: boton_seleccionar.config(state=tk.DISABLED))

    def activar_boton():
        ventana_principal.after(0, lambda: boton_seleccionar.config(state=tk.NORMAL))
    ...
    def mostrar_resultados():
        ...
        if messagebox.askyesno("Proceso Finalizado", "El procesamiento de los archivos ha terminado.\n¿Desea cerrar la aplicación?", parent=ventana_principal):
            ...
            ventana_principal.after(0, lambda: [
                etiqueta_estado.config(text="Listo para seleccionar nuevos archivos."),
                barra_progreso.config(value=0),
                etiqueta_etr.config(text="Tiempo restante: --:--")
            ])
    ...
    def procesar():
        ...
        for i, archivo_actual in enumerate(lista_archivos):
            ...
            actualizar_estado(f"Procesando {i+1}/{num_archivos}: {nombre_base_archivo}")
            actualizar_barra(i)
                ...
                if not texto_limpio_para_llm:
                    ...
                    if respuesta_deepseek:
                        if "DETENER_PROCESO_CONDICION_" in respuesta_deepseek:
                            ...
                            datos_extraidos_tabla["Ruta Completa"] = archivo_actual
