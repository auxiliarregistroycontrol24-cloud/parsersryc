...
def crear_tabla_resumen_en_vivo(main_root_ref, retry_callback):
    ...
    tabla_ventana.geometry("1920x500") 
    ...
    cols = ("Archivo Origen", "Modelo Usado", "Programa al que aspira", "Plan", "Nivel", "Acciones", "Ejecutar Script", "Marcar Realizada", "Marcar Pendiente", "Marcar No Procesada")
    ...
    for col in cols:
        ...
        elif col == "Marcar No Procesada": tree.column(col, width=150, minwidth=130, anchor='center')
    ...
    def on_click(event):
        ...
        if column_id_str == '#6': 
            ...
            base_dir = os.path.dirname(pdf_path)
            no_procesadas_dir = os.path.join(base_dir, "NO PROCESADAS")

            try:
                os.makedirs(no_procesadas_dir, exist_ok=True)
                shutil.move(pdf_path, no_procesadas_dir)
                messagebox.showinfo("Éxito", f"Archivo movido a:\n{no_procesadas_dir}", parent=tabla_ventana)
                tree.delete(row_id)
            except Exception as e:
                messagebox.showerror("Error al Mover", f"Ocurrió un error al mover el archivo a 'NO PROCESADAS':\n{e}", parent=tabla_ventana)
...
def worker_process_file(archivo_queue, ventana_principal, tabla_treeview, barra_progreso, etiqueta_estado, num_archivos, model_a_usar, worker_id, is_retry=False):
    ...
    while not archivo_queue.empty():
        ...
        if nombre_base_archivo.lower().endswith('.pdf'):
            texto_extraido = extraer_texto_pdf_con_pymupdf(archivo_actual)
...
def iniciar_procesamiento_en_hilo(boton_seleccionar, boton_ver_tabla, etiqueta_estado, barra_progreso, etiqueta_etr, ventana_principal, model_vars):
    ...
    def iniciar_reintento():
        ...
        for i, file_info in enumerate(files_to_retry_info):
            ...
            if tabla_treeview and tabla_treeview.exists(path):
                valores_reintento = (os.path.basename(path), "Reintentando...", "...", "...", "...", "Abrir Archivo", "", "(Realizada)", "(Pendiente)", "(No Procesada)")
    ...
    try:
        ...
        for ruta_archivo in lista_ordenada:
            ...
            valores_iniciales = (nombre_archivo, "Pendiente...", "", "", "", "📂 Ver Archivo", "", "✓ Realizada", "⏸ Pendiente", "🚫 No Procesada")
...
if __name__ == "__main__":
    ...
    ventana_principal_app.title("Procesador de Documentos AHK v2.1 (PyMuPDF Corregido)")
    ventana_principal_app.geometry("950x550") 
